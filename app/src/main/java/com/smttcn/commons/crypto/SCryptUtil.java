...
import static com.smttcn.commons.helpers.Base64.*;
import static com.smttcn.commons.helpers.ConstantsKt.KEY_LENGTH;
import static com.smttcn.commons.helpers.ConstantsKt.KEY_SALT_LENGTH;
...
public class SCryptUtil {
     ...
     */
    public static String scrypt(String passwd, int saltLength, int keyLength, int cpuCost, int memoryCost, int Parallelization) {
        try {
            byte[] salt = new byte[saltLength];
            SecureRandom.getInstance("SHA1PRNG").nextBytes(salt);

            byte[] derived = SCrypt.scrypt(passwd.getBytes("UTF-8"), salt, cpuCost, memoryCost, Parallelization, keyLength);

            String params = Long.toString(log2(cpuCost) << 16L | memoryCost << 8 | Parallelization, 16);
     ...
     */
    public static boolean check(String passwd, String hashed, int keyLength) {
        try {
            ...
            byte[] derived1 = SCrypt.scrypt(passwd.getBytes("UTF-8"), salt, N, r, p, keyLength);
